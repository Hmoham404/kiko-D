          ...
          <thead className="sticky top-0 z-20">
            <tr className="bg-slate-700 text-white font-bold uppercase text-center">
              ...
              {DEPARTMENTS.map(dept => (
                <th colSpan={7} key={dept.storeName} className="px-2 py-2 border-r border-slate-600 text-center">
                  {dept.displayName}
                </th>
              ))}
            </tr>
            <tr className="bg-slate-50 text-slate-700 font-semibold uppercase text-center border-b border-gray-300">
              {DEPARTMENTS.flatMap(dept => [
                ...
                <th key={`${dept.storeName}-scr`} className="px-2 py-2 border-r border-slate-300 text-right min-w-[60px] text-red-600">%scrap</th>
              ])}
            </tr>
          </thead>
          <tbody>
                  ...
                  {DEPARTMENTS.flatMap(dept => {
                    ...
                    if (!item) {
                      return [
                        ...
                        <td key={`${dept.storeName}-${date}-scr`} className="px-2 py-3 border-r border-slate-200 text-right text-gray-300">-</td>
                      ];
                    }
                    ...
                    return [
                      ...
                      <td key={`${dept.storeName}-${date}-scr`} className={`px-2 py-3 border-r border-slate-200 text-right ${scrapColor}`}>{formatPct(item.scrapRate)}</td>
                    ];
                  })}
                </tr>
              );
