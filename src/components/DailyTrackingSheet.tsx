import React from 'react';
import { ProductionData, SubComponentData, useStore } from '@/store/useStore';
import { format, parseISO } from 'date-fns';
import { fr } from 'date-fns/locale';
import { buildWeeklyTargetOverrideKey, getWeekMetadata, getWeeklyCumulativeMetrics } from '@/lib/weeklyMetrics';

interface DailyTrackingSheetProps {
  data: ProductionData[];
  subComponentsData: SubComponentData[];
}

const DEPARTMENTS = [
  { storeName: 'Injection', displayName: 'INJECTION', colorClass: 'bg-gradient-to-r from-blue-700 to-indigo-850' },
  { storeName: 'Soudure', displayName: 'US WELDING', colorClass: 'bg-gradient-to-r from-teal-700 to-cyan-850' },
  { storeName: 'Metallisation', displayName: 'METALLISATION', colorClass: 'bg-gradient-to-r from-indigo-700 to-violet-850' },
  { storeName: 'US serigraphie', displayName: 'PRINTING', colorClass: 'bg-gradient-to-r from-violet-700 to-purple-850' },
  { storeName: 'Assemblage', displayName: 'ASSEMBLAGE', colorClass: 'bg-gradient-to-r from-fuchsia-700 to-pink-850' },
  { storeName: 'Packaging', displayName: 'PACKAGING', colorClass: 'bg-gradient-to-r from-emerald-700 to-green-850' },
];

export const DailyTrackingSheet: React.FC<DailyTrackingSheetProps> = ({ data, subComponentsData }) => {
  const weeklyTargets = useStore((state) => state.weeklyTargets);
  const uniqueDates = Array.from(new Set([...data.map((item) => item.date), ...subComponentsData.map((item) => item.date)]))
    .filter(Boolean)
    .sort();

  const injectionSeries = React.useMemo(() => {
    return uniqueDates
      .map((date) => {
        const mainItem = data.find((item) => item.department === 'Injection' && item.date === date);
        if (mainItem) return mainItem;

        const subsForDate = subComponentsData.filter((item) => item.date === date);
        if (subsForDate.length === 0) return null;

        const { weekKey, weekLabel } = getWeekMetadata(date);
        return {
          department: 'Injection',
          date,
          week: weekLabel,
          weekKey,
          target: subsForDate.reduce((sum, item) => sum + item.target, 0),
          weeklyTarget: subsForDate.reduce((sum, item) => sum + item.weeklyTarget, 0),
          actualProduction: subsForDate.reduce((sum, item) => sum + item.actualProduction, 0),
          conformQty: subsForDate.reduce((sum, item) => sum + item.conformQty, 0),
          scrapQty: subsForDate.reduce((sum, item) => sum + item.scrapQty, 0),
          progress: 0,
          gap: 0,
          scrapRate: 0,
          status: 'orange' as const,
        };
      })
      .filter(Boolean) as ProductionData[];
  }, [data, subComponentsData, uniqueDates]);

  if (uniqueDates.length === 0) {
    return (
      <div className="bg-white/80 backdrop-blur-md border border-slate-100 rounded-2xl p-12 text-center text-slate-500 shadow-xl max-w-lg mx-auto">
        <div className="w-16 h-16 bg-slate-100 rounded-full flex items-center justify-center mx-auto mb-4">
          <svg className="w-8 h-8 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 17v-2m3 2v-4m3 4v-6m2 10H3a2 2 0 002-2V5a2 2 0 00-2-2h9l2 2h6a2 2 0 012 2v12a2 2 0 01-2 2z" />
          </svg>
        </div>
        <h3 className="text-lg font-bold text-slate-800 mb-1">Aucune donnee disponible</h3>
        <p className="text-sm text-slate-400">Veuillez importer un fichier Excel pour commencer.</p>
      </div>
    );
  }

  const formatNum = (num: number) => new Intl.NumberFormat('fr-FR').format(num);
  const formatPct = (num: number) => `${(num * 100).toFixed(1)}%`;

  const formatDateDisplay = (dateStr: string) => {
    try {
      return format(parseISO(dateStr), 'dd/MM/yyyy');
    } catch {
      return dateStr;
    }
  };

  const getDayOfWeek = (dateStr: string) => {
    try {
      return format(parseISO(dateStr), 'eee', { locale: fr });
    } catch {
      return '';
    }
  };

  const getDepartmentSeries = (department: string) => {
    if (department === 'Injection') {
      return injectionSeries;
    }
    return data.filter((item) => item.department === department);
  };

  const getDepartmentItemForDate = (department: string, date: string) => {
    return getDepartmentSeries(department).find((item) => item.date === date);
  };

  const getDepartmentMetricsForDate = (department: string, date: string) => {
    const series = getDepartmentSeries(department);
    return getWeeklyCumulativeMetrics(
      series,
      date,
      (item) => weeklyTargets[buildWeeklyTargetOverrideKey('department', department, item.weekKey)] ?? item.weeklyTarget
    );
  };

  return (
    <div className="bg-white/90 backdrop-blur-lg rounded-2xl shadow-xl border border-slate-200/80 overflow-hidden transition-all duration-300 hover:shadow-2xl">
      <div className="p-5 bg-gradient-to-r from-slate-900 via-slate-850 to-slate-900 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 border-b border-slate-850 text-white">
        <div className="flex items-center space-x-3">
          <div className="w-3 h-7 bg-red-500 rounded-full animate-pulse"></div>
          <div>
            <span className="text-sm font-extrabold tracking-widest uppercase bg-gradient-to-r from-white to-slate-300 bg-clip-text text-transparent">
              Vue Hebdomadaire Cumulee
            </span>
            <p className="text-[10px] text-slate-400 font-semibold tracking-wide uppercase mt-0.5">Pilotage semaine par semaine</p>
          </div>
        </div>
        <span className="text-xs text-slate-400 font-medium px-3 py-1 bg-white/5 rounded-full border border-white/10 backdrop-blur-sm">
          Cumul semaine base sur Conform Qty / Objectif hebdo
        </span>
      </div>

      <div className="overflow-x-auto max-h-[650px] overflow-y-auto scrollbar-thin scrollbar-thumb-slate-300 scrollbar-track-slate-100">
        <table className="w-full text-xs text-left border-collapse">
          <thead className="sticky top-0 z-20 shadow-[0_4px_10px_rgba(0,0,0,0.05)]">
            <tr className="bg-slate-900 text-white font-bold uppercase text-center border-b border-slate-850">
              <th className="px-4 py-5 border border-slate-800 sticky left-0 bg-slate-950 z-30 min-w-[140px] text-center tracking-wider font-black text-xs shadow-[2px_0_5px_rgba(0,0,0,0.2)]">
                JOUR / DATE
              </th>
              <th colSpan={7} className="px-2 py-3 border border-slate-800 text-center tracking-widest font-black text-xs text-white bg-gradient-to-r from-red-700 to-red-900">
                <span className="drop-shadow-md italic">TOTAL FACTORY GLOBAL</span>
              </th>
              {DEPARTMENTS.map((dept) => (
                <th key={dept.storeName} colSpan={7} className={`px-2 py-3 border border-slate-800 text-center tracking-widest font-black text-xs text-white ${dept.colorClass}`}>
                  <span className="drop-shadow-md">{dept.displayName}</span>
                </th>
              ))}
            </tr>

            <tr className="bg-slate-50 text-slate-700 font-bold uppercase text-center border-b border-slate-200">
              <th className="px-4 py-3 border-r border-slate-200 sticky left-0 bg-slate-50 z-10"></th>
              <th className="px-2 py-3 border-r border-red-200 text-center min-w-[95px] text-[9px] text-red-600 font-black tracking-wider bg-red-50/50">OBJ WEEK</th>
              <th className="px-2 py-3 border-r border-red-200 text-center min-w-[90px] text-[9px] text-red-600 font-black tracking-wider bg-red-50/50">PROD CUM</th>
              <th className="px-2 py-3 border-r border-red-200 text-center min-w-[95px] text-[9px] text-red-600 font-black tracking-wider bg-red-50/50">CONFORM CUM</th>
              <th className="px-2 py-3 border-r border-red-200 text-center min-w-[95px] text-[9px] text-red-600 font-black tracking-wider bg-red-50/50">% PROGRESS</th>
              <th className="px-2 py-3 border-r border-red-200 text-center min-w-[90px] text-[9px] text-red-600 font-black tracking-wider bg-red-50/50">GAP</th>
              <th className="px-2 py-3 border-r border-red-200 text-center min-w-[85px] text-[9px] text-red-600 font-black tracking-wider bg-red-50/50">SCRAP CUM</th>
              <th className="px-2 py-3 border-r border-slate-300 text-center min-w-[85px] text-[9px] text-red-600 font-black tracking-wider bg-red-50/50">% SCRAP</th>

              {DEPARTMENTS.flatMap((dept) => [
                <th key={`${dept.storeName}-tgt`} className="px-2 py-3 border-r border-slate-200 text-center min-w-[95px] text-[9px] text-slate-500 font-black tracking-wider">OBJ WEEK</th>,
                <th key={`${dept.storeName}-prd`} className="px-2 py-3 border-r border-slate-200 text-center min-w-[90px] text-[9px] text-slate-500 font-black tracking-wider">PROD CUM</th>,
                <th key={`${dept.storeName}-cnf`} className="px-2 py-3 border-r border-slate-200 text-center min-w-[95px] text-[9px] text-slate-500 font-black tracking-wider">CONFORM CUM</th>,
                <th key={`${dept.storeName}-prg`} className="px-2 py-3 border-r border-slate-200 text-center min-w-[95px] text-[9px] text-slate-500 font-black tracking-wider">% PROGRESS</th>,
                <th key={`${dept.storeName}-gap`} className="px-2 py-3 border-r border-slate-200 text-center min-w-[90px] text-[9px] text-slate-500 font-black tracking-wider">GAP</th>,
                <th key={`${dept.storeName}-scp`} className="px-2 py-3 border-r border-slate-200 text-center min-w-[85px] text-[9px] text-red-500 font-black tracking-wider">SCRAP CUM</th>,
                <th key={`${dept.storeName}-scr`} className="px-2 py-3 border-r border-slate-300 text-center min-w-[85px] text-[9px] text-red-500 font-black tracking-wider">% SCRAP</th>,
              ])}
            </tr>
          </thead>

          <tbody>
            {uniqueDates.map((date, rowIndex) => {
              const isEven = rowIndex % 2 === 0;
              const availableMetrics = DEPARTMENTS.map((dept) => {
                const item = getDepartmentItemForDate(dept.storeName, date);
                if (!item) return null;
                return {
                  department: dept.storeName,
                  metrics: getDepartmentMetricsForDate(dept.storeName, date),
                };
              }).filter(Boolean) as Array<{
                department: string;
                metrics: {
                  weekKey: string;
                  weeklyTarget: number;
                  actualProduction: number;
                  conformQty: number;
                  scrapQty: number;
                  progress: number;
                  gap: number;
                  scrapRate: number;
                };
              }>;

              const globalWeeklyTarget = availableMetrics.reduce((sum, entry) => sum + entry.metrics.weeklyTarget, 0);
              const globalActual = availableMetrics.reduce((sum, entry) => sum + entry.metrics.actualProduction, 0);
              const globalConform = availableMetrics.reduce((sum, entry) => sum + entry.metrics.conformQty, 0);
              const globalScrap = availableMetrics.reduce((sum, entry) => sum + entry.metrics.scrapQty, 0);
              const globalProgress = globalWeeklyTarget > 0 ? globalConform / globalWeeklyTarget : 0;
              const globalGap = globalWeeklyTarget - globalConform;
              const globalScrapRate = globalActual > 0 ? globalScrap / globalActual : 0;

              const getStatusColor = (progress: number) => {
                if (progress >= 1) return 'text-emerald-600 bg-emerald-50';
                if (progress >= 0.8) return 'text-amber-600 bg-amber-50';
                return 'text-red-600 bg-red-50';
              };

              return (
                <tr key={date} className={`border-b border-slate-100 hover:bg-indigo-50/30 transition-colors duration-200 ${isEven ? 'bg-white' : 'bg-slate-50/50'}`}>
                  <td className="px-4 py-3 border-r border-slate-200 font-extrabold text-slate-800 sticky left-0 bg-white z-10 text-center whitespace-nowrap shadow-[4px_0_8px_-3px_rgba(0,0,0,0.1)]">
                    <span className="bg-slate-100 text-slate-800 px-2 py-1 rounded-md text-[11px] border border-slate-200/60 inline-block font-mono">
                      {formatDateDisplay(date)}
                    </span>
                    <span className="text-[10px] text-slate-400 font-bold lowercase tracking-wide block mt-1">
                      {getDayOfWeek(date)}
                    </span>
                  </td>

                  <td className="px-2 py-3 border-r border-slate-200 text-center font-bold text-slate-900 bg-red-50/20">{formatNum(globalWeeklyTarget)}</td>
                  <td className="px-2 py-3 border-r border-slate-200 text-center font-bold text-slate-900 bg-red-50/20">{formatNum(globalActual)}</td>
                  <td className="px-2 py-3 border-r border-slate-200 text-center font-bold text-slate-900 bg-red-50/20">{formatNum(globalConform)}</td>
                  <td className={`px-2 py-3 border-r border-slate-200 text-center font-black ${getStatusColor(globalProgress)}`}>{formatPct(globalProgress)}</td>
                  <td className={`px-2 py-3 border-r border-slate-200 text-center font-bold ${globalGap <= 0 ? 'text-emerald-600' : 'text-red-600'}`}>
                    {globalGap > 0 ? `+${formatNum(globalGap)}` : formatNum(globalGap)}
                  </td>
                  <td className="px-2 py-3 border-r border-slate-200 text-center font-bold text-red-500 bg-red-50/10">{formatNum(globalScrap)}</td>
                  <td className={`px-2 py-3 border-r border-slate-300 text-center font-bold ${globalScrapRate > 0.05 ? 'text-red-600 bg-red-50' : 'text-slate-500'}`}>{formatPct(globalScrapRate)}</td>

                  {DEPARTMENTS.flatMap((dept) => {
                    const item = getDepartmentItemForDate(dept.storeName, date);
                    if (!item) {
                      return [
                        <td key={`${dept.storeName}-${date}-tgt`} className="px-3 py-3 border-r border-slate-100 text-center text-slate-300 font-mono">-</td>,
                        <td key={`${dept.storeName}-${date}-prd`} className="px-3 py-3 border-r border-slate-100 text-center text-slate-300 font-mono">-</td>,
                        <td key={`${dept.storeName}-${date}-cnf`} className="px-3 py-3 border-r border-slate-100 text-center text-slate-300 font-mono">-</td>,
                        <td key={`${dept.storeName}-${date}-prg`} className="px-3 py-3 border-r border-slate-100 text-center text-slate-300 font-mono">-</td>,
                        <td key={`${dept.storeName}-${date}-gap`} className="px-3 py-3 border-r border-slate-100 text-center text-slate-300 font-mono">-</td>,
                        <td key={`${dept.storeName}-${date}-scp`} className="px-3 py-3 border-r border-slate-100 text-center text-slate-300 font-mono">-</td>,
                        <td key={`${dept.storeName}-${date}-scr`} className="px-3 py-3 border-r border-slate-300 text-center text-slate-300 font-mono">-</td>,
                      ];
                    }

                    const metrics = getDepartmentMetricsForDate(dept.storeName, date);
                    let progressBg = 'bg-red-50 text-red-700 border-red-200';
                    let progressBarColor = 'bg-red-500';

                    if (metrics.progress >= 1) {
                      progressBg = 'bg-emerald-50 text-emerald-700 border-emerald-200';
                      progressBarColor = 'bg-emerald-500';
                    } else if (metrics.progress >= 0.8) {
                      progressBg = 'bg-amber-50 text-amber-700 border-amber-200';
                      progressBarColor = 'bg-amber-500';
                    }

                    const gapBg = metrics.gap > 0 ? 'bg-red-50 text-red-700 border-red-200' : 'bg-emerald-50 text-emerald-700 border-emerald-200';

                    return [
                      <td key={`${dept.storeName}-${date}-tgt`} className="px-3 py-3 border-r border-slate-100 text-right text-slate-600 font-semibold font-mono">
                        {formatNum(metrics.weeklyTarget)}
                      </td>,
                      <td key={`${dept.storeName}-${date}-prd`} className="px-3 py-3 border-r border-slate-100 text-right font-bold text-slate-800 font-mono">
                        {formatNum(metrics.actualProduction)}
                      </td>,
                      <td key={`${dept.storeName}-${date}-cnf`} className="px-3 py-3 border-r border-slate-200 text-right text-emerald-600 font-extrabold font-mono bg-emerald-50/20">
                        {formatNum(metrics.conformQty)}
                      </td>,
                      <td key={`${dept.storeName}-${date}-prg`} className="px-3 py-3 border-r border-slate-100 text-center font-mono">
                        <div className={`inline-flex flex-col items-center justify-center px-2 py-1 rounded-md border ${progressBg} font-extrabold w-full shadow-sm`}>
                          <span>{formatPct(metrics.progress)}</span>
                          <div className="w-full bg-slate-200 h-1 rounded-full mt-1 overflow-hidden">
                            <div className={`${progressBarColor} h-1 rounded-full`} style={{ width: `${Math.min(metrics.progress * 100, 100)}%` }}></div>
                          </div>
                        </div>
                      </td>,
                      <td key={`${dept.storeName}-${date}-gap`} className="px-3 py-3 border-r border-slate-100 text-center font-mono">
                        <span className={`inline-block px-2 py-0.5 rounded border ${gapBg} font-extrabold shadow-sm`}>
                          {metrics.gap > 0 ? '+' : ''}{formatNum(metrics.gap)}
                        </span>
                      </td>,
                      <td key={`${dept.storeName}-${date}-scp`} className="px-3 py-3 border-r border-slate-100 text-right text-red-600 font-bold font-mono bg-red-50/10">
                        {formatNum(metrics.scrapQty)}
                      </td>,
                      <td key={`${dept.storeName}-${date}-scr`} className="px-3 py-3 border-r border-slate-300 text-right text-slate-500 font-medium font-mono">
                        <span className={metrics.scrapRate > 0.05 ? 'text-red-600 font-bold animate-pulse' : ''}>
                          {formatPct(metrics.scrapRate)}
                        </span>
                      </td>,
                    ];
                  })}
                </tr>
              );
            })}
          </tbody>
        </table>
      </div>
    </div>
  );
};
