...
import { buildWeeklyTargetOverrideKey, getWeeklyCumulativeMetrics } from '@/lib/weeklyMetrics';
  ...
  if (!data || data.length === 0) {
    return (
      <div className="bg-white/80 backdrop-blur-md border border-slate-100 rounded-2xl p-12 text-center text-slate-500 shadow-xl max-w-lg mx-auto">
        ...
        <h3 className="text-lg font-bold text-slate-800 mb-1">Aucune donnee de sous-composants</h3>
    ...
    <div className="bg-white/90 backdrop-blur-lg rounded-2xl shadow-xl border border-slate-200/80 overflow-hidden transition-all duration-300 hover:shadow-2xl">
      <div className="p-5 bg-gradient-to-r from-slate-900 via-slate-850 to-slate-900 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 border-b border-slate-850 text-white">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <span className="text-sm font-extrabold tracking-widest uppercase bg-gradient-to-r from-white to-slate-300 bg-clip-text text-transparent">
              Details Injection (Base, Cover, Insert)
            </span>
            <p className="text-[10px] text-slate-400 font-semibold tracking-wide uppercase mt-0.5">Suivi cumule hebdomadaire</p>
          </div>
        </div>
        <span className="text-xs text-slate-400 font-medium px-3 py-1 bg-white/5 rounded-full border border-white/10 backdrop-blur-sm">
          Base sur Conform Qty / Objectif hebdo
        </span>
      </div>
        ...
        <table className="w-full text-xs text-left border-collapse">
          <thead className="sticky top-0 z-20 shadow-[0_4px_10px_rgba(0,0,0,0.05)]">
            <tr className="bg-slate-900 text-white font-bold uppercase text-center border-b border-slate-850">
              <th className="px-4 py-5 border border-slate-800 sticky left-0 bg-slate-950 z-30 min-w-[140px] text-center tracking-wider font-black text-xs shadow-[4px_0_8px_rgba(0,0,0,0.2)]">JOUR / DATE</th>
              {COMPONENTS.map((comp) => (
                <th key={comp.name} colSpan={7} className={`px-2 py-3 border border-slate-800 text-center tracking-widest font-black text-xs text-white ${comp.colorClass}`}>
            ...
            <tr className="bg-slate-50 text-slate-700 font-bold uppercase text-center border-b border-slate-200">
              ...
              {COMPONENTS.flatMap((comp) => [
                <th key={`${comp.name}-tgt`} className="px-2 py-3 border-r border-slate-200 text-center min-w-[95px] text-[9px] text-slate-500 font-black tracking-wider">OBJ WEEK</th>,
                <th key={`${comp.name}-prd`} className="px-2 py-3 border-r border-slate-200 text-center min-w-[90px] text-[9px] text-slate-500 font-black tracking-wider">PROD CUM</th>,
                <th key={`${comp.name}-cnf`} className="px-2 py-3 border-r border-slate-200 text-center min-w-[95px] text-[9px] text-slate-500 font-black tracking-wider">CONFORM CUM</th>,
                ...
                <th key={`${comp.name}-scp`} className="px-2 py-3 border-r border-slate-200 text-center min-w-[85px] text-[9px] text-red-500 font-black tracking-wider">SCRAP CUM</th>,
              ...
              return (
                <tr key={date} className={`border-b border-slate-100 hover:bg-indigo-50/30 transition-colors duration-200 ${isEven ? 'bg-white' : 'bg-slate-50/50'}`}>
                  <td className="px-4 py-3 border-r border-slate-200 font-extrabold text-slate-800 sticky left-0 bg-white z-10 text-center whitespace-nowrap shadow-[4px_0_8px_-3px_rgba(0,0,0,0.1)]">
                    <span className="bg-slate-100 text-slate-800 px-2 py-1 rounded-md text-[11px] border border-slate-200/60 inline-block font-mono">{formatDateDisplay(date)}</span>
                    <span className="text-[10px] text-slate-400 font-bold lowercase tracking-wide block mt-1">{getDayOfWeek(date)}</span>
                  </td>

                  {COMPONENTS.flatMap((comp) => {
                    const series = data.filter((item) => item.component === comp.name);
                    const item = series.find((entry) => entry.date === date);
                    ...
                    return [
                      <td key={`${comp.name}-${date}-tgt`} className="px-3 py-3 border-r border-slate-100 text-right text-slate-600 font-semibold font-mono">{formatNum(metrics.weeklyTarget)}</td>,
                      <td key={`${comp.name}-${date}-prd`} className="px-3 py-3 border-r border-slate-100 text-right font-bold text-slate-800 font-mono">{formatNum(metrics.actualProduction)}</td>,
                      <td key={`${comp.name}-${date}-cnf`} className="px-3 py-3 border-r border-slate-200 text-right text-emerald-600 font-extrabold font-mono bg-emerald-50/20">{formatNum(metrics.conformQty)}</td>,
                      ...
                      <td key={`${comp.name}-${date}-scp`} className="px-3 py-3 border-r border-slate-100 text-right text-red-600 font-bold font-mono bg-red-50/10">{formatNum(metrics.scrapQty)}</td>,
                      <td key={`${comp.name}-${date}-scr`} className="px-3 py-3 border-r border-slate-300 text-right text-slate-500 font-medium font-mono">
                        <span className={metrics.scrapRate > 0.05 ? 'text-red-600 font-bold animate-pulse' : ''}>{formatPct(metrics.scrapRate)}</span>
                      </td>,
                    ];
                  })}
                </tr>
